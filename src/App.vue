<template>
  <div id="app">
    ...
    <main>
      <div class="leftalign">
        <div class="synth-menu">
          ...
          <v-select
            ...
            :options="synthTones"
            :value="synthTone"
            @input="(instrument) => updateInstrument(instrument)"
            name="Instrument"
            class="instrument"
          />
        </div>
      ...
      <div class="flexbox">
        <PlayButton
          @change="onPlayButtonClick"
          :disabled="disabled"
          :chord-loop="chordLoop"
          :selected-chords="selectChords"
          :chord="chord"
          :beat="beat"
        />
        <ButtonPickChords
          @pickChordsClicked="onPickChordsChildClick"
          :disabled="disabled"
        />
        <JazzifyButton
          :chord="chord"
          @jazzifyClicked="onJazzifyChildClick"
          :disabled="disabled"
        />
      </div>
      <div class="keymenu">
      ...
      <div class="dropdowns">
        <Dropdown
          @chord-toggle="onChordToggle"
          :select-chords="selectChords"
          :chord-names="getChordNames"
          :beat="beat"
          :disabled="disabled"
          :chord-loop="chordLoop"
          :positions="positions"
...
export default {
  ...
  computed: {
    selectChords() {
      const chordArray = selectChords(
        this.positions,
        this.scale.key,
        this.scale.mode
      ).map((chord) => s11.chord.create(chord, 3));
      const coverted = chordArray.map((chordInfo) =>
        chordInfo.chord.map((note) => {
  ...
  methods: {
    ...
    jazzifyChord(position) {
      ...
      } else if (modeNum >= 0.16 && modeNum <= 0.25) {
        newPosition.mode = "aug";
      } else if (modeNum > 0.25 && modeNum <= 0.62) {
        newPosition.mode = "M";
      } else {
        newPosition.mode = "m";
      }
    ...
    convertNotes(allChords) {
      // runs convertNoteToKeyboard function on each chord in array, which changes double flats or double sharps into keyboard readable notes.
      const convertedChords = allChords.map((chord) =>
        chord.map((note) => {
          return convertNoteToKeyboard(note);
        })
      );
