<template>
  <section id="chord-select">
    ...
    >
      <h3
        @click="toggleChords(position, index)"
        @keypress="toggleChords(position, index)"
        :class="beatId(position.beat)"
      >
        {{ chordNames[index] }}
      </h3>
      <v-select
      ...
      <v-select
        :clearable="false"
        :disabled="disabled"
        @input="chordLoop.dispose()"
        :options="modes"
        :placeholder="position.mode"
        :value="position.mode"
        v-model="position.mode"
      />
    </div>
  </section>
</template>
